...
function nextWeekday(s) {
  ...
  do {
    z.setUTCDate(z.getUTCDate() + 1);
  } while ([0, 6].includes(z.getUTCDay()));
...
function daysBetween(a, b) {
  ...
  return Number.isFinite(x.getTime()) && Number.isFinite(y.getTime())
    ? Math.round((y - x) / 86400000)
    : null;
}
...
function isStock(type, symbol, name) {
  ...
  if (
    /(fund|fon|cash|nakit|repo|bond|fixed|borc|borç|lease|kira|deposit|mevduat|money|other)/.test(
      t
    )
  ) {
    return false;
...
function holding(row) {
  const fundCode = code(
    first(row, ["fund_code", "fundCode", "fund", "fundcode"])
  );
  ...
  const name = String(
    first(
      row,
      ["name", "asset_name", "security_name", "title"],
      symbol || ""
    )
  ).trim();
  ...
  const assetType = String(
    first(
      row,
      ["asset_type", "assetType", "type", "category", "asset_class"],
      ""
    )
  ).trim();
...
function sbConfig() {
  ...
  if (!url || !key) {
    throw new Error("SUPABASE_URL veya Supabase anahtari eksik");
  }
...
async function sb(path, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      `Supabase ${options.method || "GET"} ${path} HTTP ${
        response.status
...
async function priceYahoo(symbol) {
  ...
  for (const host of [
    ...
    try {
      ...
      if (!x) {
        throw new Error(
          j?.chart?.error?.description || "empty_chart"
        );
      }

      const ts = x.timestamp || [];
      const adj =
        x?.indicators?.adjclose?.[0]?.adjclose || [];
      const raw =
        x?.indicators?.quote?.[0]?.close || [];
      ...
      if (!pair) {
        throw new Error("insufficient_chart_points");
      }
...
async function loadHoldings() {
  const raw = await sb(
    "fund_holdings?select=*&limit=5000"
  );
...
async function loadPerformance() {
  try {
    const raw = await sb(
      `prediction_performance?select=*` +
        `&model=eq.${MODEL}` +
        `&fund_code=in.(${FUNDS.join(",")})` +
        `&status=in.(closed,shock_closed)` +
...
function fundStats(rows) {
  ...
  return {
    ...
    latestFundActualChange:
      num(r[0]?.daily_change),

    recentAverageFundChange:
      rnd(
        avg(changes),
        4
      ),
...
function nonStockProxy(
  ...
  if (
    /(fixed|bond|borc|borç|lease|kira)/.test(
      t
    )
  ) {
    return 0.04;
  ...
  async function handler(
    ...
    try {
      if (
        ...
        return res
          ...
          .json({
            ...
            error:
              "Yetkisiz istek. Manuel test icin ?manual=finscope kullanin.",
          });
      }
      ...
      for (
        ...
        let stockContribution =
          stockObserved;

        let coverageExpansion = 1;
        ...
        predictions.push({
          ...
          engine:
            "direct_market_price_bridge",

          officialHoldingsReportDate:
            OFFICIAL_DATES[f],
          ...
          latestFundPriceDate:
            fs.latestFundPriceDate,

          latestFundActualChange:
            fs.latestFundActualChange,

          recentAverageFundChange:
            fs.recentAverageFundChange,
          ...
          performanceAverageAbsoluteError:
            learn.averageAbsoluteError,

          performanceDirectionHitRate:
            learn.directionHitRate,
          ...
          pricingDiagnostics: {
            ...
            method:
              "Yahoo chart v8 direct per symbol; adjusted close preferred; query1->query2 fallback",
          },
      ...
      return res
        ...
        .json({
          ...
          pricingRule:
            "Katki = portfoy agirligi/100 x Yahoo chart gunluk fiyat degisimi. Adjusted close tercih edilir; gecmis performans sadece sinirli kalibrasyondur.",

          reliabilityRule:
            `Stock coverage %${MIN_RELIABLE_STOCK_COVERAGE} altindaysa agresif tahmin engellenir.`,
